...
| ADR-004 | Real-time components and their interfaces: one record per frame through ingest → decode → GM → tracker → context → stage → gating → modules → outputs → alert service; stubs so every owner builds against replayed inputs (PF-Q2-13) | proposed | 2026-09-15 | contract / stage events / infra |
