...
| `module_dataset_needs.md` + `.json` | `scripts/module_dataset_needs.py` from `module_consumption.json` + `pf/rt/declarations.json` | Valentyn (module-porter) | **done 24.09**: per module, which classes are the check itself, which are the aircraft / the traffic / synthesised rows, and which models it already carries (ours vs stock COCO) — the list for planning datasets and for taking a check off the shared GM |
| `roadmap_move_analysis.md` + `.json` | `scripts/roadmap_move_analysis.py` from the roadmap page `docs/planning/RV_scope_timeline.html` plus `rt_module_cost.json`, `rt_environment.json`, `module_dataset_needs.json`, `critical_label_queue.json` | Valentyn (pm-coordinator + module-porter) | **done 25.09**: which checks are the cheapest to move into minutes without a trigger change (4 of them sit in hours in the plan today), what blocks the rest, and which critical check fits the edge slot instead of the nose gear chocks |
| `rt_priority_plan.md` + `.json` | `scripts/rt_priority_plan.py` (greedy on the marginal cost of the next check, waves A–E) | Valentyn (pm-coordinator + module-porter) | **done 25.09**: the order to move the checks into minutes and what each wave needs from the platform; `--write-roadmap` writes the same order and modes into `docs/planning/RV_scope_timeline.html` (24 checks in minutes, 4 in hours, 2 on the edge) |
